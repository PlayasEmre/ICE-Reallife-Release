...
function easterEggShapeHit ( hit, dim )
	...
	if getElementType ( hit ) == "player" and dim then
		...
		MtxSetElementData ( hit, "easterEggs", MtxGetElementData ( hit, "easterEggs" ) + 1 )
...
function buyEasterBonus ( item )
	...
	if item == "premium" then
		...
		if MtxGetElementData ( player, "premium" ) == false then
			MtxSetElementData ( player, "easterEggs", eggs - 60 )
			setPremiumData(player,30,3)
			outputChatBox ( "Du hast für 30 Tage Premium geschenkt gekriegt", player, 0, 253, 0 )
		else
			outputChatBox ( "Du hast schon bereits Premium", player, 200, 0, 0 )
		end
		else
	...
	if item == "money" then
		if eggs >= 50 then
			MtxSetElementData ( player, "easterEggs", eggs - 50 )
			outputChatBox ( "Du kriegst 2000€ geschenkt", player, 0, 253, 0 )
			MtxSetElementData ( player, "money",MtxGetElementData ( player, "money" ) + 2000 )
		  else
			outputChatBox ( "Du hast nicht genug Kuerbisse!", player, 200, 0, 0 )
		  end
    end
	if item == "Süßigkeit" then
		if eggs >= 10 then
			MtxSetElementData ( player, "easterEggs", eggs - 10 )
