-- ==========================================================
-- I. KONFIGURATION (Konfiguration): Ändern Sie nur die Werte in diesem Block
-- ==========================================================
...
-- Fügt das Argument 'isManualCheck' hinzu, um die Logik zu steuern
function checkUpdate(isManualCheck) 
	isManualCheck = isManualCheck or false -- Standardmäßig nicht manuell
	...
	outputDebugString(DEBUG_TAG.."DEBUG: URL-Anfrage: "..url)
	...
	fetchRemote(url, function(data, err)
		if err == 0 then
			...
			if RemoteVersion > version then
				...
				if AUTO_DOWNLOAD_ENABLED or isManualCheck then -- Startet sofort bei Auto-Download ODER manuellem Check
				...
				-- Prüft und stoppt den alten Erinnerungs-Timer, bevor ein neuer gesetzt wird
				...
				updateTimer = setTimer(function()
					if RemoteVersion > version then
						...
						-- Stoppe den Timer, wenn die Version in der Zwischenzeit aktualisiert wurde
						outputChatBox(DEBUG_TAG.." [Info] Die Ressource ist aktuell (Version: "..version.."). Stoppe Erinnerungs-Timer.", root, 50, 255, 50)
						if isTimer(updateTimer) then killTimer(updateTimer) end
						updateTimer = false
					end
				end, notice_interval_ms, 0)
				
				-- Gib die erste Update-Meldung sofort aus
				outputChatBoxToAdmins(DEBUG_TAG.." [WICHTIG] Ein Update ("..RemoteVersion..") ist verfügbar! Befehl: /update "..RES_NAME, 255, 50, 50)
...
-- ==========================================================
-- V. Automatischer Timer Start (VERBESSERT)
-- ==========================================================

if AUTO_CHECK_ENABLED then
	
	-- VERBESSERUNG: Sicherstellen, dass der Timer nur einmal läuft
...
addCommandHandler("update", function(player, cmd, targetResourceName)
	...
	if isPermit then
		...
		-- Wenn das Update bereits bekannt ist und verfügbar ist, starte Download sofort
		...
		-- Wenn das Update nicht verfügbar ist oder RemoteVersion 0 ist, starte manuellen Check
		outputChatBoxToAdmins(DEBUG_TAG..getPlayerName(player) .. " versucht, "..RES_NAME.." manuell zu prüfen.", 100, 100, 255)
		checkUpdate(true) -- Ruft checkUpdate als manuelle Prüfung auf
	else
...
function DownloadFiles()
	...
	fetchRemote(url, function(data, err, path)
		if err == 0 then
			...
			if err == 2 then errorString = "Netzwerk-Timeout / Verbindung blockiert (Firewall?)" end
			if err == 404 then errorString = "Datei nicht gefunden (URL / Branch ist falsch)" end
			if err == -1 then errorString = "ACL Access Denied (Benötigt function.fetchRemote)" end
...
function DownloadFinish()
	...
	-- Den periodischen Timer stoppen, da die Ressource gleich neu startet
