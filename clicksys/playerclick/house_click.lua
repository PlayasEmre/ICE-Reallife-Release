...
function showHouseGui_func ( amount )
	...
	if source == lp then
		if gWindow["houseMenue"] then
			...
			gWindow["houseMenue"] = guiCreateStaticImage(screenwidth/2-173/2,screenheight/2-215/2,173,215,":"..getResourceName(getThisResource()).."/images/background.png",false)
			guiSetAlpha(gWindow["houseMenue"],1)
			gImage["house$"] = guiCreateStaticImage(0.3295,0.1349,0.1734,0.1349,":"..getResourceName(getThisResource()).."/images/dollar.png",true,gWindow["houseMenue"])
