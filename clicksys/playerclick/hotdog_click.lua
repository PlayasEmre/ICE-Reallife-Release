...
function giveHotDogGui()
	...
	if gWindow["sellHotdog"] then
		...
		gLabel["hotdog$Symbol"] = guiCreateLabel(0.4652,0.3148,0.0535,0.1481,""..Tables.waehrung.."",true,gWindow["sellHotdog"])
		...
		addEventHandler("onClientGUIClick", gButton["sellHotdog"],
			function()
				triggerServerEvent ( "sellhotdog", localPlayer, localPlayer, "", guiGetText ( gEdit["hotdogPrice"] ) )
			end
		)
	end
end
...
function HotdogLoadMenue()
	...
	if gWindow["hotdogBuyin"] then
		...
		gLabel["hotdogPrice"] = guiCreateLabel(0.0593,0.7273,0.5556,0.1515,"1 "..Tables.waehrung.." / Stueck",true,gWindow["hotdogBuyin"])
		...
		addEventHandler("onClientGUIClick", gButton["hotdogLoad"],
			function()
				triggerServerEvent ( "buyhotdogs", localPlayer, localPlayer, math.abs ( math.floor ( tonumber ( guiGetText ( gEdit["hotdogAmount"] ) ) ) ) )
				showCursor ( false )
