...
function highnoon_func ( player, cmd, name, summe, runden )
	...
	if name and name ~= player then
		...
		if summe then
			if summe <= MtxGetElementData ( player, "money" ) and MtxGetElementData ( name, "money" ) then
				...
				if runden then
					if runden >= 1 and runden <= 10 then
						...
						if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 5 and getDistanceBetweenPoints3D ( x1, y1, z1, x3, y3, z3 ) <= 5 then
							...
							MtxSetElementData ( player, "highNoonQuestion", getPlayerName ( name ) )
							MtxSetElementData ( name, "highNoonQuestion", getPlayerName ( player ) )
							MtxSetElementData ( name, "highNoonSumme", summe )
							MtxSetElementData ( name, "highNoonRunden", runden )
						else
...
function accepthighnoon_func ( player, cmd )
	...
	local name = MtxGetElementData ( player, "highNoonQuestion" )
	if name then
		local name = getPlayerFromName ( name )
		if name then
			setElementDimension ( player, 0 )
			setElementDimension ( name, 0 )
			local summe = MtxGetElementData ( player, "highNoonSumme" )
			if summe <= MtxGetElementData ( player, "money" ) and MtxGetElementData ( name, "money" ) then
				if not isPedDead ( player ) and not isPedDead ( name ) and not MtxGetElementData ( player, "isInHighNoon" ) and not MtxGetElementData ( name, "isInHighNoon" ) then
					removePedFromVehicle ( player )
					removePedFromVehicle ( name )
					if cmd == "accepthighnoon" then
						MtxSetElementData ( player, "highNoonWin", 0 )
						MtxSetElementData ( name, "highNoonWin", 0 )
					end
					local x1, y1, z1 = getElementPosition ( player )
					local x2, y2, z2 = getElementPosition ( name )
					local x3, y3, z3 = getElementPosition ( highnoonIcon )
					if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 5 and getDistanceBetweenPoints3D ( x1, y1, z1, x3, y3, z3 ) <= 5 then
						local runden = MtxGetElementData ( player, "highNoonRunden" )
						...
						local dim = tonumber ( MtxGetElementData ( player, "playerid" ) )
						takeAllWeapons ( player )
						setElementDimension ( player, dim )
						setPedArmor ( player, 0 )
						MtxSetElementData ( player, "isInHighNoon", true )
						MtxSetElementData ( name, "highNoonRunden", runden )
						...
						MtxSetElementData ( name, "isInHighNoon", true )
						...
						if cmd == "accepthighnoon" then
							...
							MtxSetElementData ( name, "money", MtxGetElementData ( name, "money" ) - summe )
						end
					else
						outputChatBox ( "Du und dein Gegner muessen am Waffenladen sein!", player, 125, 0, 0 )
					end
...
function highNoonDraw ( p1, p2 )
	...
	local runden = MtxGetElementData ( p1, "highNoonRunden" )
	local summe = MtxGetElementData ( p1, "highNoonSumme" )
	if (MtxGetElementData ( p1, "isInHighNoon" ) and MtxGetElementData ( p2, "isInHighNoon" )) then
		...
		elseif isPedDead ( p1 ) then
			...
			if isElement ( p2 ) then
				...
				MtxSetElementData ( p2, "highNoonWin", MtxGetElementData ( p2, "highNoonWin" ) + 1 )
		...
		elseif isPedDead ( p2 ) then
			...
			if isElement ( p1 ) then
				...
				MtxSetElementData ( p1, "highNoonWin", MtxGetElementData ( p1, "highNoonWin" ) + 1 )
...
function checkIfItIsOver ( p1, p2 )
	...
	local runden = MtxGetElementData ( p1, "highNoonRunden" ) - 1
	if runden then
		if runden >= 1 then
			...
			MtxSetElementData ( p1, "highNoonRunden", runden )
			MtxSetElementData ( p2, "highNoonRunden", runden )
			setElementDimension ( p1, 1 )
			setElementDimension ( p2, 1 )
			...
			elseif MtxGetElementData ( p1, "highNoonWin" ) < MtxGetElementData ( p2, "highNoonWin" ) then
				MtxSetElementData ( p2, "money", MtxGetElementData ( p2, "money" ) + (summe*2) )
				...
				MtxSetElementData ( p2, "money", MtxGetElementData ( p2, "money" ) + summe )
				MtxSetElementData ( p1, "money", MtxGetElementData ( p1, "money" ) + summe )
...
function endHighNoon ( p1, p2, summe )
	...
	if isElement ( p1 ) then
		if MtxGetElementData ( p1, "isInHighNoon" ) then
	...
	elseif isElement ( p2 ) then
		if MtxGetElementData ( p2, "isInHighNoon" ) then
			MtxSetElementData ( p2, "highNoonWin", MtxGetElementData ( p2, "highNoonWin" ) + 1 )
			endHighNoonForPlayer ( p2, true )
			setElementPosition ( p2, -2617.2189941406, 209.10488891602, 4.4970808029175 )
			setElementDimension ( p2, 1 )
...
function endHighNoonForPlayer ( player, bool )
	...
	takeAllWeapons ( player )
	setElementModel ( player, tonumber ( MtxGetElementData ( player, "skinid" ) ) )
	setElementDimension ( player, 0 )
	setPlayerNametagShowing ( player, true )
	if bool then
		MtxSetElementData ( player, "isInHighNoon", false )
	end
end