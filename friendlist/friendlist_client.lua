...
function showFriendlistSelf()
	...
	if gWindow["friendlistMenue"] then
		...
		gWindow["friendlistMenue"] = guiCreateStaticImage(screenwidth/2-364/2,120,364,282,":"..getResourceName(getThisResource()).."/images/background.png",false)
		...
		addEventHandler("onClientGUIClick", gButton["friendAdd"],
			function()
				triggerServerEvent ( "addFriend", getRootElement(), localPlayer, guiGetText ( gEdit["friendName"] ) )
			end
		)
				...
				if row then
					if player then
						...
						if MtxGetElementData ( player, "married" ) == 1 then
							partner = MtxGetElementData ( player, "marwith" )
						else
							partner = "Nicht Verheiratet"
						end
						guiSetText ( gLabel["friendMarryValue"], partner )
					else
