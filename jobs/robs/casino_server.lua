...
if getElementType(hitElement) == "player" and (dim) then
	if isPedInVehicle ( hitElement ) == false then
		if isEvil( hitElement ) then
			...
			MtxSetElementData ( hitElement, "isinRob", true )
...
if getElementType(hitElement) == "player" and (dim) then
	if isPedInVehicle ( hitElement ) == false then
		...
		MtxSetElementData ( hitElement, "isinRob", false )
		if isEvil(hitElement) then
		...
		if getElementType(hitElement) == "player" and (dim) then
			if robstate == "ausstehend" then
				if isEvil( hitElement ) then
					...
					MtxSetElementData ( hitElement, "money", MtxGetElementData ( hitElement, "money" ) + pricebrutto )
					...
					MtxSetElementData ( hitElement, "boni", MtxGetElementData ( hitElement, "boni" ) + (pricebrutto/200) )
...
function sohier(player)
	if not isPedDead ( player ) and getElementData ( player, "loggedin" ) == 1 then
		if defusestate == "machend" then
			...
			MtxSetElementData ( player, "boni", MtxGetElementData ( player, "boni" ) + (pricebrutto/100) )
