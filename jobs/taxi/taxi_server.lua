...
function taxiEnter_func ( player, seat, jacked )
	if seat == 0 then
		if MtxGetElementData ( player, "job" ) == "taxifahrer" then
...
function taxiPayCheck_func ()
	...
	if isElement ( player ) and isElement ( driver ) then
		if MtxGetElementData ( player, "money" ) >= taxiPricePerInterval then
...
function accept_taxi ( player, cmd, arg )
	if arg == "taxi" then
		local veh = getPedOccupiedVehicle ( player )
		if veh and isVehicleTaxiLightOn ( veh ) and MtxGetElementData ( player, "job" ) == "taxifahrer" then
