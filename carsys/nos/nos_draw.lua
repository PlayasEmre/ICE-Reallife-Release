...
function nosRender ()
	if drawNos and getPedOccupiedVehicle ( lp ) and getVehicleUpgradeOnSlot ( getPedOccupiedVehicle ( lp ), 8 ) then
		...
		dxDrawImageSection ( x, y + fixDist - distToTop, width, -height, 1, distToTop, width, height, ":"..getResourceName(getThisResource()).."/images/carsys/nos/nos.png" )
	else
		drawNos = false
	...
	if getPedOccupiedVehicleSeat ( lp ) == 0 then
		if getElementData ( veh, "nitro" ) > 0 then
			...
			if not drawNos then
				...
				width, height = NosBottleWidth, NosBottleHeight
				nosBottle = guiCreateStaticImage ( x, y, width, height, ":"..getResourceName(getThisResource()).."/images/carsys/nos/bottle.png", false )
			end
			return nil
		end
	end
