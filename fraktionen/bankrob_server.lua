...
function info_bank_timer ()
	...
	if bank_i == 0 then
		...
		if isElementWithinColShape ( bank_robber, bank_rob_col ) then
			...
			MtxSetElementData ( bank_robber, "money", MtxGetElementData (bank_robber, "money" ) + bank_money )
			robber_wasted ()
...
function bank_rob ( player, _ )
	...
	if isEvil ( player ) and MtxGetElementData ( player, "rang" ) >= 2 and isElementWithinColShape ( player, bank_rob_col ) then
