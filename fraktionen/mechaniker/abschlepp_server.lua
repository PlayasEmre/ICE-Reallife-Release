				...
				if getElementModel ( hitElement ) == 525 then
					...
					if towedVehicle ~= false then
						if MtxGetElementData(towedVehicle, "owner") then
							...
							dbExecAsync ( handler, "UPDATE vehicles SET Spawnpos_X=?, Spawnpos_Y=?, Spawnpos_Z=?, Spawnrot_X=?, Spawnrot_Y=?, Spawnrot_Z=?, Farbe=?, Paintjob=?, Benzin=?, Distance=?, Beschlagnahmt=? WHERE UID=? AND Slot=?",Spawnpos_X, Spawnpos_Y, Spawnpos_Z, Spawnrot_X, Spawnrot_Y, Spawnrot_Z, color, Paintjob, Benzin, Distance, Beschlagnahmt, playerUID[pname], slot)
			...
			local x, y, z = getElementPosition ( vehicle )
				if MtxGetElementData(vehicle, "Beschlagnahmt") == 1 then
					if MtxGetElementData(player, "money") >= 2000 then
						...
						dbExecAsync(handler, "UPDATE vehicles SET Beschlagnahmt=? WHERE UID=? AND slot=?", 0, playerUID[getPlayerName(player)], carslot)
