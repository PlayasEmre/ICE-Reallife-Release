...
local function addTotalschadenToVehicle(veh, owner, Slot)
    if veh then
        if owner and Slot then
			...
			dbExecAsync ( handler, "UPDATE vehicles SET totalschaden = ?  WHERE UID=? AND Slot=?", 1, playerUID[owner], Slot ) 
	...
	if isMechaniker(player) and veh and target then
		if MtxGetElementData(veh, "totalschaden") == 1 then
			if MtxGetElementData(target,"money") >= 1500 or MtxGetElementData(player,"money") >= 2000 then
				...
				dbExecAsync(handler, "UPDATE vehicles SET totalschaden = ? WHERE UID=? AND Slot=?", 0, playerUID[MtxGetElementData(veh, "owner")], MtxGetElementData(veh, "carslotnr_owner"))
