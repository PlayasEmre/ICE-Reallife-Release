		...
		if getElementData(lp,"fraktion")==1 then
			if(getElementData(lp,"ElementClicked")==false)then
				...
				addEventHandler("onDgsMouseClick",Duty.Button[2],
					function(btn,state)
						if btn=="left" and state=="up" then
							triggerServerEvent("go:sapd_duty3",lp,lp)
						end
					end,
				false)
				...
				addEventHandler("onDgsMouseClick",Duty.Button[3],
					function(btn,state)
						if btn=="left" and state=="up" then
							triggerServerEvent("gooff:sapd_duty3",lp,lp)
						end
					end,
				false)
...
addEvent("open:sapd_duty_gui3",true)
addEventHandler("open:sapd_duty_gui3",getRootElement(),sapdDuty_GUI3)
