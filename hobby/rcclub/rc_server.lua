...
function joinRCClub ()
	...
	local player = client
	if MtxGetElementData ( player, "money" ) >= 250 then
		MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) - 250 )
		infobox ( player, "Du bist jetzt\nMitglied im RC-Club!", 5000, 0, 125, 0 )
		MtxSetElementData ( player, "club", "rc" )
	else
...
function buyRCVehicleVan ( veh )
	...
	if price then
		if MtxGetElementData ( player, "club" ) == "rc" then
...
function rcVanEnter ( player, seat )
	...
	local veh = source
	local rcID = MtxGetElementData ( veh, "rcVehicle" )
	if rcID then
...
function userc ( player )
	...
	if veh then
		if getPedOccupiedVehicleSeat ( player ) > 1 then
			local rcID = MtxGetElementData ( veh, "rcVehicle" )
			if rcID then
				if MtxGetElementData ( veh, "rcVehicleInUse" ) then
					...
					if getElementModel ( veh ) == 459 then
						...
						MtxSetElementData ( player, "RCVanSeat", getPedOccupiedVehicleSeat ( player ) )
						MtxSetElementData ( player, "RCVan", veh )
						--local dummy = createPed ( getElementModel ( player ), 0, 0, 0 )
						--MtxSetElementData ( dummy, "player", player )
...
function rcVehicleExited ( veh )
	...
	local van = MtxGetElementData ( player, "RCVan" )
	local seat = MtxGetElementData ( player, "RCVanSeat" )
...
function rcVehicleDummyWasted ()
	...
	local ped = source
	local player = MtxGetElementData ( dummy, "player" )
	destroyElement ( ped )
