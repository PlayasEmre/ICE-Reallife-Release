...
function isFounderOfGang ( player, gangFounderID )
	...
	if isElement ( player ) then
		if gangFounderID then
			if getPlayerGangRank ( player ) >= 3 and gangFounderID == MtxGetElementData ( player, "housekey" ) then
				return true
			end
		elseif getPlayerGangRank ( player ) >= 3 and getPlayerGang ( player ) == MtxGetElementData ( player, "housekey" ) then
			return true
		end
	else
...
function deleteGang ( id )
	...
	gangData[id] = nil
	MtxSetElementData ( _G["HouseNR"..id], "gangHQOf", false )
end